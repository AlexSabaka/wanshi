# wanshi — Roadmap (post-release, prioritized)

> The forward plan, ordered by importance — synthesized from the whole benchmark-and-release arc by
> Dove (2026-06-23) and promoted to the canonical roadmap. Two organizing principles: **(1) validate the
> thesis before over-refining extraction; (2) foundational + cheap-high-value before big-research.**
>
> **Status (2026-06-24):** `@wanshi-kg/wanshi` v0.1.0 shipped; the benchmark stream closed with the
> precision + typed-extraction wins; the frontend provenance/trace/source-view (Phase A+B) landed. What
> follows is what's *next* — see [`PROJECT_STATE.md`](./PROJECT_STATE.md) for what currently *is*.
...
1. **KBLaM/LoRA injection spike.** Take ~1K real triples (lesson-transcript graph) → inject via KBLaM or a
   ...
   delivers.** *Status: a Phase-9 LoRA-MLX spike showed triples train in, but end-to-end
   answering-from-injected-facts is **not yet validated** — so this stands as #1. Effort: spike.
   Dependency: none (current extraction is "good enough" to test).*
...
10. **Measurement extraction integration.** Wire the `metricat` measurement-extraction sibling in as the
    opt-in, corpus-gated enrichment. *Effort: low.*
...
14. **Human review / curation UI.** The #2 cross-persona want; **in progress** (frontend stream): Sable's
    design convergence, the provenance/trust UI, and the source/provenance view (Phase A+B) have landed;
    the remaining piece is the accept/reject/edit/merge **write-back** (needs a persistence-model
    decision). *Continue. Effort: ongoing.*
...
## Hand-back
Injection has been spiked but **not validated end-to-end** — so #1 stands: it's the cheapest way to learn
whether the whole thing delivers, and it should anchor everything below it. Then exact dedup (free win), then the
